...
#pragma mark 邓永军 添加长按手势
        UILongPressGestureRecognizer *longGesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(handlongGesture:)];
...
- (void)handlongGesture:(UILongPressGestureRecognizer *)longGesture{
    if (longGesture.state == UIGestureRecognizerStateBegan &&  [self.photoViewDelegate respondsToSelector:@selector(photoViewlongGesture:)]) {
        [self.photoViewDelegate photoViewlongGesture:self];
    }
}
